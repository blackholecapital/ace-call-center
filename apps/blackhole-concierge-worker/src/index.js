...
async function processPreliminaryEstimate(env,payload={}){
  ...
  const email=await callBinding(env.EMAIL,"https://email.internal/internal/send",{contactId,contact,messageType:"ace-preliminary-estimate",quote,requirements});
  if(email?.ok!==true)throw new Error(email?.error||`Estimate email failed (${email?.status||"unknown"})`);
  const patch={stage:"Estimate Sent",callStatus:"In progress",estimateStatus:"Sent",estimateNumber:quote.estimateNumber,estimateSentAt:createdAt,estimateValidUntil:validUntil,estimatedMonthlyTotal:quote.monthlyTotal||0,selectedProduct:quote.serviceName||contact.selectedProduct||contact.interest||""};
  await persistContact(env,contact,patch);
  await updateDashboardContact(env,contactId,patch);
  await emit(env,{type:"estimate.sent",contactId,estimateNumber:quote.estimateNumber,messageId:email.messageId||"",productName:quote.serviceName||"",monthlyTotal:Number(quote.monthlyTotal||0)});
  return{ok:true,contactId,quote,email};
}

async function scheduleDelivery(env,payload={}){
  const contactId=payload.contactId||"",contact=await resolveContact(env,contactId,payload);if(!contactId||!contact?.id)throw new Error("Implementation scheduling requires a valid contact");if(String(contact.documentStatus||"").toLowerCase()!=="signed")throw new Error("Agreement must be signed before implementation scheduling");if(!googleCalendarConfigured(env))throw new Error("Google Calendar is not configured");
  const startIso=String(payload.startIso||payload.start||"");if(!startIso||Number.isNaN(new Date(startIso).getTime()))throw new Error("A valid implementation start time is required");const durationMinutes=Math.max(30,Number(payload.durationMinutes||120));const endIso=String(payload.endIso||new Date(new Date(startIso).getTime()+durationMinutes*60000).toISOString());if(!(await isSlotAvailable(env,startIso,endIso)))return{ok:false,conflict:true,error:"That implementation slot is no longer available"};
...
export default { async fetch(request,env,ctx){
  ...
  if(url.pathname==="/api/health")return Response.json({ok:true,service:env.TENANT_ID==="ace-host"?"ace-concierge-worker":"blackhole-concierge-worker",health:"online",runtime:"edge",tenant:tenantContext(env),docusign:docusignConfigured(env)?"configured":"not-configured",googleCalendar:googleCalendarConfigured(env)?"configured":"not-configured",googleCalendarTimeZone:googleCalendarTimeZone(env)});
