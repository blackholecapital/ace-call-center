...
async function processPreliminaryEstimate(env,payload={}){
  ...
  const email=await callBinding(env.EMAIL,"https://email.internal/internal/send",{contactId,contact,messageType:"ace-preliminary-estimate",quote,requirements,callback:{callNowUrl,persistent:true,source:"estimate-email"}});
...
async function processSalesHandoff(env,payload={}){
  ...
  await persistContact(env,contact,patch);
  await updateDashboardContact(env,contactId,patch);
  await emit(env,{type:"sales.handoff.created",contactId,callSid:String(payload.callSid||""),message:`Sales handoff created: ${reason}`,reason,requirements,source:String(payload.source||"ace-voice-worker")});
